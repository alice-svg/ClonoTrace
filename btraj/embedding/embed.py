...
class AntiBERTyEmbedder:
    ...
    def compute_bcr_embeddings(
        ...
        # 对缺失的 heavy/light，用 0 vector 替代（可调整策略）
        mask_heavy_nan = adata.obs[heavy_col].isna().to_numpy()
        mask_light_nan = adata.obs[light_col].isna().to_numpy()
        heavy_emb[mask_heavy_nan] = 0.0
        light_emb[mask_light_nan] = 0.0
        ...
        if pca_dim is not None and pca_dim < X_concat.shape[1]:
            print(f"Applying PCA to reduce AntiBERTy BCR embedding to {pca_dim} dimensions...")
            pca = PCA(n_components=pca_dim, random_state=42)
            X_bcr = pca.fit_transform(X_concat)  # [N, pca_dim]
        else:
            X_bcr = X_concat  # [N, 2*hidden_dim]
